...
const ChatView = ({ id }) => {
    const wrapperClasses = classNames(styles.emptyWrapper, styles.emptyWrapperRight)
    ...
    const filteredChats = chats.data.filter(chat => chat?.chatName?.toLowerCase()?.includes(value.toLowerCase()));
    ...
    const isEmptyScreen = chats && chats?.data?.length === 0 && !chats.isLoading && chats.error === null;
    ...
    const handleSendMessage = (values) => {
        dispatch(sendMessage({ chatId: selectedChat.id, data: { text: values.text, attachments: [] } }))
    }
        ...
        <>
            {!isEmptyScreen ?
                <div className={styles.chatMainWrapper}>
                    <SelectChatAside filteredChats={filteredChats} searchActive={inputActive} fetchMoreData={fetchMoreData} hasMore={chats.hasNext} input={
                    ...
                    {(!id && !selectedChat) ? <div className={wrapperClasses}>
